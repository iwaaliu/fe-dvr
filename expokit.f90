...
!----------------------------------------------------------------------|
!----------------------------------------------------------------------|
      subroutine ZGPADM(ideg,m,t,H,ldh,awsp,lwsp,ipiv,iexph,ns,iflag) 
                                                                        
      implicit none 
      double precision t 
      integer          ideg, m, ldh, lwsp, iexph, ns, iflag, ipiv(m) 
      complex*16       H(ldh,m), awsp(lwsp) 
...
!                                                                       
      ...
      do j = 1,m 
         do i = 1,m 
            awsp(i) = awsp(i) + ABS( H(i,j) ) 
         enddo 
      enddo 
      hnorm = 0.0d0 
      do i = 1,m 
         hnorm = MAX( hnorm,DBLE(awsp(i)) ) 
      enddo 
      hnorm = ABS( t*hnorm ) 
...
!                                                                       
      ...
      awsp(icoef) = ONE 
      do k = 1,ideg 
         awsp(icoef+k) = (awsp(icoef+k-1)*DBLE( i-k ))/DBLE( k*(j-k) ) 
      enddo 
!                                                                       
!---  H2 = scale2*H*H ...                                               
!                                                                       
      call ZGEMM( 'n','n',m,m,m,scale2,H,ldh,H,ldh,ZERO,awsp(ih2),m ) 
!                                                                       
!---  initialise p (numerator) and q (denominator) ...                  
!                                                                       
      cp = awsp(icoef+ideg-1) 
      cq = awsp(icoef+ideg) 
      do j = 1,m 
         do i = 1,m 
            awsp(ip + (j-1)*m + i-1) = ZERO 
            awsp(iq + (j-1)*m + i-1) = ZERO 
         enddo 
         awsp(ip + (j-1)*(m+1)) = cp 
         awsp(iq + (j-1)*(m+1)) = cq 
      enddo 
...
!                                                                       
  ...
  100 continue 
      iused = iodd*iq + (1-iodd)*ip 
      call ZGEMM( 'n','n',m,m,m, ONE,awsp(iused),m,                      &
     &             awsp(ih2),m, ZERO,awsp(ifree),m )                      
      do j = 1,m 
         awsp(ifree+(j-1)*(m+1)) = awsp(ifree+(j-1)*(m+1))+awsp(icoef+k-1) 
      enddo 
      ip = (1-iodd)*ifree + iodd*ip 
      iq = iodd*ifree + (1-iodd)*iq 
      ifree = iused 
...
!                                                                       
      if ( iodd.ne.0 ) then 
         call ZGEMM( 'n','n',m,m,m, scale,awsp(iq),m,                    &
     &                H,ldh, ZERO,awsp(ifree),m )                        
         iq = ifree 
      else 
         call ZGEMM( 'n','n',m,m,m, scale,awsp(ip),m,                    &
     &                H,ldh, ZERO,awsp(ifree),m )                        
         ip = ifree 
      endif 
      call ZAXPY( mm, -ONE,awsp(ip),1, awsp(iq),1 ) 
     ! print*,'if=',iflag
      call ZGESV( m,m, awsp(iq),m, ipiv, awsp(ip),m, iflag ) 
      if(iflag.ne.0) print*,'iflag=',iflag
      if ( iflag.ne.0 ) stop 'Problem in ZGESV (within ZGPADM)' 
      call ZDSCAL( mm, 2.0d0, awsp(ip), 1 ) 
      do j = 1,m 
         awsp(ip+(j-1)*(m+1)) = awsp(ip+(j-1)*(m+1)) + ONE 
      enddo 
      iput = ip 
      if ( ns.eq.0 .and. iodd.ne.0 ) then 
         call ZDSCAL( mm, -1.0d0, awsp(ip), 1 ) 
         goto 200 
      endif 
!                                                                       
!--   squaring : exp(t*H) = (exp(t*H))^(2^ns) ...                       
!                                                                       
      ...
      do k = 1,ns 
         iget = iodd*ip + (1-iodd)*iq 
         iput = (1-iodd)*ip + iodd*iq 
         call ZGEMM( 'n','n',m,m,m, ONE,awsp(iget),m, awsp(iget),m,       &
     &                ZERO,awsp(iput),m )                                
         iodd = 1-iodd 
      enddo 
  200 continue 
